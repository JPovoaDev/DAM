...
class AIAssistantGemini(override val properties: Properties) : AIAssistant {
    ...
    override val temperatureAPI = "TEMPERATURE"
    override val max_tokenAPI = "MAX_TOKEN"
    override val sentimentAPI = "SENTIMENT"
    ...
    override fun buildRequest(prompt: String): Request {
        ...
        val requestBody = JSONObject()
            ...
            .apply {
                //adicionamos ao json a temperatura e os maxtokens
                val genConfig = JSONObject()
                temperature?.let { genConfig.put("temperature", it) }
                max_token?.let { genConfig.put("maxOutputTokens", it) }
                if (genConfig.length() > 0) put("generationConfig", genConfig)
            }
