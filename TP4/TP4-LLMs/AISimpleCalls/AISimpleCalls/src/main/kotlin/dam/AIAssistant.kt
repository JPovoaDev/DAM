...
interface AIAssistant {
    ...
    /**
     * A temperatua e os tokens maximos que o modelo de AI vai
     * usar
     */

    val temperatureAPI: String
    val max_tokenAPI:String
    ...
    /**
     * O boolean que diz ao modelo de AI
     * Se é para analizar o sentimento ou
     * para responder a prompt normalmente
     */
    val sentimentAPI:String
    ...
    /**
     * Uma vez que adicionamos ao config.properties para conseguirmos ter acesso as variaveis temos que as
     * inicializar aqui na interface
     */
    val temperature:Double?
        get() = properties.getProperty(temperatureAPI)?.toDoubleOrNull()

    val max_token:Int?
        get()=properties.getProperty(max_tokenAPI)?.toIntOrNull()


    val sentiment: Boolean
        get() = properties.getProperty(sentimentAPI)?.toBooleanStrictOrNull()?:false
    /**
    ...
    fun buildPrompt(input: String): String {
        return if (sentiment){
            ...
            6. Positive
            7. Very Positive
            Responde only in this JSON format :
            {
                " rating ": value ,
                " justification ": value
            }
            With no other extra text
