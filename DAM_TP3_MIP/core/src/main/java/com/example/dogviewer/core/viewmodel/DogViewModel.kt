...
class DogViewModel(private val repository: DogRepository) : ViewModel() {
    ...
    /**
     * Gets the total number of favorite dogs currently saved in the repository.
     */
    ...
    /**
     * Gets the list of favorite dogs.
     */
    ...
    /**
     * Checks if a given image is in the favorites list.
     */
    fun isFavorite(item: ImageItem): Boolean {
        return repository.getFavorites().any { it.url == item.url }
    }
    ...
    /**
     * Helper for Compose Navigation: Finds a dog by ID in current images or favorites.
     */
    fun getDogById(id: String): ImageItem? {
        return _images.value?.find { it.id == id } ?: repository.getFavorites().find { it.id == id }
    }
}
